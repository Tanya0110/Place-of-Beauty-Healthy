...
import { INITIAL_BOOKINGS, DIKIDI_COMPANY_ID, DIKIDI_API_KEY, DIKIDI_WIDGET_URL, FULL_PRICE_LIST } from './data';
import { sendTelegramNotification } from './services/telegramService';
import { AdminPanel } from './components/AdminPanel'; // Import AdminPanel (Assuming it handles isAdmin logic internally or unused for now if not requested to change, but referencing just in case)
...
const Header: React.FC<{ 
  ...
  isDarkMode: boolean;
  toggleTheme: () => void;
}> = ({ currentPage, onNavigate, openBooking, isDarkMode, toggleTheme }) => {
...
const Footer: React.FC<{ onNavigate: (page: string) => void; openPrivacy: () => void }> = ({ onNavigate, openPrivacy }) => (
...
const App: React.FC = () => {
       ...
       try {
         ...
         if (res.ok) {
           ...
           setReviews(data.map((r: any, i: number) => ({
             ...
             text: r.text,
             rating: r.rating || 5,
             date: new Date().toLocaleDateString('ru-RU') // Default date
           })));
         }
       ...
       try {
         ...
         if (res.ok) {
           ...
           setGalleryItems(data.map((item: any, i: number) => ({
             ...
             aspectRatio: 'square', // Default aspect ratio for backend images
             imageScale: 1
           })));
         }
       } catch(e) { console.log('Portfolio fetch failed', e); }
       ...
       try {
         ...
         if (res.ok) {
           ...
           setBlogPosts(data.map((post: any, i: number) => ({
             ...
             comments: [],
             tags: [],
             isOriginal: true // Simple view
           })));
         }
       } catch(e) { console.log('Blog fetch failed', e); }
    };
  ...
  const saveReviewToBackend = async (review: Review) => {
      try {
          await fetch('http://localhost:5000/save_review', {
              method: 'POST',
          ...
          console.log("Backend not available for review save");
      }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-['Montserrat'] selection:bg-purple-200 selection:text-purple-900 transition-colors duration-300">
      ...
      <main>
        ...
        {/* Pass priceList to CertificatesPage/Modal logic if needed, currently CertificatesPage only opens modal */}
